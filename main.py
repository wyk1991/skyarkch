...
class Skygate():
    ...
    log_file_name = '/Users/bill/Downloads/skygate-main/checkin_account.txt'
    abi_json_filepath = '/Users/bill/Downloads/skygate-main/abi.json'
    erc_721_filepath = '/Users/bill/Downloads/skygate-main/erc721.json'
    ...
    def check_has_slime_nft(self):
        ...
        nft_contract = self.web3.eth.contract(address=nft_contract_address, abi=self.load_abi(self.erc_721_filepath))
    ...
    def dispatch_squead(self, data):
        ...
        tx = self.web3.eth.contract(address=address, abi=self.load_abi(self.abi_json_filepath)).functions.signin(data)
    ...
    def explore(self):
        ...
        tx = self.web3.eth.contract(address=address, abi=self.load_abi(self.abi_json_filepath)).functions.signin(1)
        tx_hash = self.make_tx(tx)
    ...
    def save(self, address, pk):
        # 保存账号到当前文件夹的checkin_account.txt文件, 格式： 地址----私钥----jwt
        log_str = f'{address}----{pk}----{self.jwt}\n'
    ...
    def checkin(self):
        ...
        if res.status_code != 200:
            raise Exception(f'{self.account.address} checkin error')  
    ...
    @staticmethod
    def changeTheExeclToTxtFile(self):
        log_file_name = '/Users/bill/Downloads/skygate-main/checkin_account.txt'
    ...
    def loginIn():
        ...
        for row in list(rows):
            ...
            for c in row:
                case.append(c.value)
            print(case)
            sg = Skygate(pk=case[1], jwt='', is_daily=True, proxy={})
            sg.login(case[0], case[1])
    ...
    def daily_checkin(proxy):
            for log_str in Skygate.iter_file(Skygate.log_file_name):
                ...
                sg = Skygate(pk=pk, jwt=jwt, is_daily=True, proxy=proxy)
                try:
                    sg.checkin()
                except Exception as e:
                    print(str(e))
...
# ==================================读取excel写入txt文档==================================
# Skygate.changeTheExeclToTxtFile()
